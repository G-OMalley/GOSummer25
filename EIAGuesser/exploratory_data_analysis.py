...
if total_nans > 0:
    ...
    print(final_df.isna().sum().sort_values(ascending=False).head())

print("\nSample of final column names:")
sample_cols = [col for col in final_df.columns if 'weather' in col][:2]
sample_cols += [col for col in final_df.columns if 'plattsfundy' in col][:2]
sample_cols += [col for col in final_df.columns if 'powerfundy' in col][:2]
sample_cols += [col for col in final_df.columns if 'fundy' in col][:2]
sample_cols += [col for col in final_df.columns if 'critextra' in col][:2]
sample_cols += [col for col in final_df.columns if 'storagechange' in col][:2]
print(sample_cols)
