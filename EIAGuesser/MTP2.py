...
def get_agg_dict(columns: pd.Index) -> dict:
    agg_dict = {}
    sum_keywords = ('_flow', '_demand', '_stor', '_prod', '_exp', '_imp', '_gen', '_hdd', '_cdd')
    for col in columns:
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_eng['gwdd'] = (df_eng.filter(like='_weather_').filter(like='_cdd').sum(axis=1) -
...
def add_advanced_features(df: pd.DataFrame, target_cols: list) -> pd.DataFrame:
    ...
    hdd_col_name = next((c for c in df_adv.columns if 'hdd' in c and 'conus' in c), None)
    cdd_col_name = next((c for c in df_adv.columns if 'cdd' in c and 'conus' in c), None)
    if hdd_col_name:
...
target_map = {
    'conus': 'lower_48_states_storage_change_bcf', 'east': 'east_region_storage_change_bcf',
    'midwest': 'midwest_region_storage_change_bcf', 'south_central': 'south_central_region_storage_change_bcf',
    'mountain': 'mountain_region_storage_change_bcf', 'pacific': 'pacific_region_storage_change_bcf'
}
target_cols = list(target_map.values())
targets_raw = targets_raw[target_cols]

missing_pct = features_daily.isnull().sum() / len(features_daily)
features_daily.drop(columns=missing_pct[missing_pct > MISSING_THRESHOLD].index, inplace=True)
...
print(f'Last EIA week: {last_report_date.date()} | Forecasting for: {pred_week_label.date()}')
...
for region, tgt_col in target_map.items():
    ...
    base_models = [
        ...
        lgb.LGBMRegressor(n_estimators=200, random_state=RANDOM_STATE, n_jobs=1, verbose=-1)
    ]
    ...
    for train_idx, test_idx in eval_tscv.split(X_all_np):
        ...
        oof_preds = get_oof_predictions(X_train, y_train, [clone(m) for m in base_models], TimeSeriesSplit(n_splits=5))
        
        cloned_meta_model = clone(meta_model)
        cloned_meta_model.fit(oof_preds, y_train)
        
        test_base_preds = np.column_stack([m.predict(X_test) for m in trained_base_models])
    ...
    final_oof_preds = get_oof_predictions(X_all_np, y_all_np, [clone(m) for m in base_models], TimeSeriesSplit(n_splits=5))
    ...
    pred_base_preds = np.column_stack([m.predict(pred_X.values) for m in final_stack['base']])
    next_pred = final_stack['meta'].predict(pred_base_preds)[0]

    prior_base_preds = np.column_stack([m.predict(X_all.iloc[[-1]].values) for m in final_stack['base']])
...
try:
    l48_est = res_df.loc[('Lower 48', 'ManualStack'), f'{date_label(pred_week_label)} Est']
