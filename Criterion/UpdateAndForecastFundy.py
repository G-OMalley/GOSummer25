...
def process_fundy_data(script_mode, engine, mapping_df_param, output_csv_name_param):
    ...
    if script_mode == "ACTUALS":
        # For actuals, output_csv_name_param is the direct output filename
        ...
        # For forecasts, derive the forecast filename from the base output name
        name_part, ext_part = os.path.splitext(output_csv_name_param)
        output_csv_name = f"{name_part}Forecast{ext_part}" # e.g., FundyForecast.csv or CriterionExtraForecast.csv
        ticker_column_in_mapping = 'Forecast Ticker'
        print_process_type = "Forecast Generation"
        current_start_date_from_scratch = None # Forecasts don't use a fixed historical start date
    else:
    ...
    try:
        ...
        if script_mode == "ACTUALS":
            for item_name in df_original_output['item'].unique():
                if item_name not in processed_items_from_mapping: 
                    print(f"\nItem '{item_name}' from {output_csv_name} was not in mapping file. Preserving its original data.")
                    updated_item_data_list.append(df_original_output[df_original_output['item'] == item_name])
        ...
        item_to_region_map_from_fetched = df_after_fetch[df_after_fetch['region'].notna()][['item', 'region']].drop_duplicates('item').set_index('item')['region'].to_dict()
        ...
        try:
            ...
            if 'Item' not in fundy_mapping_df.columns:
                ...
                process_fundy_data(
                    script_mode="ACTUALS",
                    engine=engine,
                    mapping_df_param=fundy_mapping_df, # Pass the loaded DataFrame
                ...
                process_fundy_data(
                    ...
                    engine=engine,
                    mapping_df_param=fundy_mapping_df, # Pass the loaded DataFrame
                    output_csv_name_param='Fundy.csv' # Base name, function will append 'Forecast'
                )
        except FileNotFoundError:
        ...
        try:
            ...
            if 'Item' not in extra_mapping_df.columns:
                ...
                process_fundy_data(
                    script_mode="ACTUALS",
                    engine=engine,
                    mapping_df_param=extra_mapping_df, # Pass the loaded DataFrame
                ...
                process_fundy_data(
                    ...
                    engine=engine,
                    mapping_df_param=extra_mapping_df, # Pass the loaded DataFrame
                    output_csv_name_param='CriterionExtra.csv' # Base name, function will append 'Forecast'
                )
        except FileNotFoundError:
